...
| `preview.spec.mjs`            | Preview mode: limitations list, enable (instant trycloudflare URL from the tunnel log, restart callout, `testdrive.env` marker, Status badge flips to Preview), disable via the API (honest response message, marker + state cleanup), and the Status-surface Disconnect turning preview off.                                                                                                                                                                         |
| `disconnect.spec.mjs`         | Disconnect from a completed Connect setup, asserting the single-surface model: Status badge + domain + one reachability chip + ONE Disconnect button, cards demoted to pure actions behind "Switch setup method", two-click confirm, full on-disk reset (config.yml, credentials, token/domain truncation, homeserver `icann_domain` reset), domain-only save rejected afterwards.                                                                                    |
...
| `connect-authorized.spec.mjs` | Connect (browser-auth) flow: prerequisites copy on the idle card, waiting card with auth link, cert-derived subdomain picker with locked suffix and suggestion chips (fixture cert authorizes example.com), client-side subdomain validation, completion artifacts + Status surface pickup (badge, domain, single restart callout), domain-only save rejected while Connect-managed, full-hostname fallback for an unparseable cert, expired-authorization idle card. |
